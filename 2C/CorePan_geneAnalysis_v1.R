...
sink(file = paste(pre, "CorePan_pan_model_v1.txt", sep="."),append = FALSE)
...
ggsave(paste(pre, "CorePan_fitmodel_v1.pdf", sep = "."), plot = p1 , device = "pdf", width = 7,height = 7 )
...
p2 <- ggplot() +
  ...
  theme(legend.title=element_blank(),
        legend.position = c(0.9, 0.6))
ggsave(paste(pre, "CorePan_fitsmooth_v1.pdf", sep = "."), plot = p2 , device = "pdf", width = 7,height = 7 )
